...
if [[ "${SCENARIO}" == "Custom" ]]; then
    ...
    MARGIN_SHAPEFILE="${MARGIN_FOLDER}/${MARGIN_NAME}.shp"
else
    MARGIN_FOLDER="ice_margins"
    MARGIN_SHAPEFILE="${MARGIN_FOLDER}/${SCENARIO}.shp"
fi
...
gmt begin ${plot_folder}/shear_stress pdf
  ...
  gmt plot ${MARGIN_SHAPEFILE} ${R_options} ${J_options}  -Wthick,blue -BneWS
